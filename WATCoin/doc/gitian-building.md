...
*Setup instructions for a gitian build of WATCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the WATCoin
...
- [Building WATCoin](#building-watcoin)
...
Clone the git repositories for watcoin and gitian and then checkout the watcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the watcoin repository
...
To build WATCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the watcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/watcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/watcoin-project/watcoin
...
For example:
```bash
URL=https://github.com/user/watcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit watcoin=${COMMIT} --url watcoin=${URL} ../watcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit watcoin=${COMMIT} --url watcoin=${URL} ../watcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit watcoin=${COMMIT} --url watcoin=${URL} ../watcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/watcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/watcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/watcoin-build.assert
```
...
[watcoin/gitian.sigs](https://github.com/watcoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
